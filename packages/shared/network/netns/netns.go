...
func newNSWithName(baseDir, name string, pid uint32) (nsPath string, err error) {
...
func umount(target string, flags int) error {
	for i := 0; i < 50; i++ {
...
func unmountNS(path string) error {
	...
	if err := umount(path, unix.MNT_DETACH); err != nil && !os.IsNotExist(err) {
...
func NewNetNSFromPID(baseDir string, pid uint32) (*NetNS, error) {
	...
	path, err := newNSWithName(baseDir, nsName, pid)
	if err != nil {
...
// This function runs almost the same as `ip netns add`.
//
// Sometimes we do not want to specify basedir, we just want a named namesapce.
// NOTE (huang-jl) that at least on Ubuntu, the netns created under /run/netns,
// so we use it here
func NewNetNSNamed(name string) (*NetNS, error) {
	path, err := newNSWithName(bindMountPath, name, 0)
	if err != nil {
		return nil, fmt.Errorf("failed to setup netns: %w", err)
	}
	return &NetNS{path: path}, nil
...
func LoadNetNSNamed(name string) *NetNS {
	return &NetNS{path: path.Join(bindMountPath, name)}
}
